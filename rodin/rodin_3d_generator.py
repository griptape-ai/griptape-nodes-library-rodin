...
class Rodin3DGenerator(ControlNode):
    def __init__(self, **kwargs) -> None:
        ...
        self.add_parameter(
            ParameterList(
                name="images",
                input_types=["ImageUrlArtifact", "list[ImageUrlArtifact]", "str"],
                default_value=[],
            ...
            try:
                ...
                while retry_count < max_retries:
                    ...
                    try:
                        ...
                        logger.debug(f"⚠️ Status check failed (attempt {retry_count}/{max_retries}): {e!s}")
                ...
                try:
                    ...
                    logger.debug(f"❌ _download_results failed: {e!s}")
                    raise
                ...
                try:
                    ...
                    logger.debug(f"❌ _process_downloads failed: {e!s}")
                    raise
                ...
                logger.debug(f"💥 generate_3d_model function failed: {e!s}")
                raise Exception(f"Rodin 3D generation failed: {e!s}")
    ...
    def _download_results(self, api_key: str, task_uuid: str) -> dict:
        ...
        try:
            ...
            logger.debug(f"❌ Download API REQUEST ERROR: {e!s}")
            raise Exception(f"Download API failed: {e!s}")
    ...
    def _process_downloads(self, download_response: dict) -> GLTFUrlArtifact:
        ...
        for index, item in enumerate(download_items):
            ...
            try:
                ...
                raise Exception(f"Failed to download {original_name}: {e!s}")
            except Exception as e:
                raise Exception(f"Failed to save file {original_name}: {e!s}")
