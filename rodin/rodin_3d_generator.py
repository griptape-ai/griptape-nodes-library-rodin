...
class Rodin3DGenerator(ControlNode):
    ...
    def process(self) -> AsyncResult:
        def generate_3d_model() -> GLTFUrlArtifact:
            try:
                ...
                logger.debug(
                    f"📷 Images retrieved: {type(images_input)} with {len(images_input) if images_input else 0} items"
                )
                ...
                logger.debug(
                    f"📋 Response keys: {list(task_response.keys()) if isinstance(task_response, dict) else 'not dict'}"
                )
                ...
                logger.debug(
                    f"✅ subscription_key extracted: {subscription_key[:8] if subscription_key else 'None'}..."
                )
                ...
                status_headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
                ...
                while retry_count < max_retries:
                    ...
                    try:
                        ...
                        status_response = requests.post(
                            status_url, headers=status_headers, json=status_data, timeout=30
                        )
                        ...
                        if all_done:
                            ...
                            logger.debug("⏳ Jobs not done yet, continuing polling...")

                    except requests.exceptions.RequestException as e:
                        logger.debug(f"⚠️ Status check failed (attempt {retry_count}/{max_retries}): {str(e)}")
                        if retry_count >= max_retries:
                            raise Exception(f"Failed to check status after {max_retries} attempts: {e}") from e
                        continue
                ...
                try:
                    ...
                    logger.debug(
                        f"✅ _download_results returned: {type(download_response)} with keys: {list(download_response.keys()) if isinstance(download_response, dict) else 'not dict'}"
                    )
                    logger.debug(
                        f"✅ Download URLs received! Response keys: {list(download_response.keys()) if isinstance(download_response, dict) else type(download_response)}"
                    )
                ...
                raise Exception(f"Rodin 3D generation failed: {str(e)}") from e
    ...
    def _submit_task(self, api_key: str, images_input: list[ImageUrlArtifact], prompt: str) -> dict:
        ...
        if bbox_condition:
            try:
                ...
                raise ValueError(
                    "Bounding box dimensions must be comma-separated numbers (e.g., 1.0,2.0,3.0)"
                ) from None
        ...
        logger.debug(
            f"📁 Files details: {[(name, filename, content_type) for name, (filename, content, content_type) in files]}"
        )
    ...
    def _download_results(self, api_key: str, task_uuid: str) -> dict:
        ...
        try:
            ...
            logger.debug("📋 Download response parsed successfully")
            ...
            raise Exception("Download API timeout after 60 seconds") from None
        except requests.exceptions.RequestException as e:
            logger.debug(f"❌ Download API REQUEST ERROR: {str(e)}")
            raise Exception(f"Download API failed: {str(e)}") from e
    ...
    def _process_downloads(self, download_response: dict) -> GLTFUrlArtifact:
        ...
        for _index, item in enumerate(download_items):
            ...
            try:
                ...
                raise Exception(f"Download timeout for {original_name} after 120 seconds") from None
            except requests.exceptions.RequestException as e:
                raise Exception(f"Failed to download {original_name}: {str(e)}") from e
            except Exception as e:
                raise Exception(f"Failed to save file {original_name}: {str(e)}") from e
        ...
        model_artifact = GLTFUrlArtifact(
            value=primary_static_url, name=primary_original_name or "rodin_3d_model", metadata=metadata
        )
